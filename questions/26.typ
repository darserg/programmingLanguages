...
(defmacro cons-stream (a str)
  `(cons ,a (delay ,str))
```
*Опр*. Протечка макроса - это ситуация, когда макрос не раскрывает код, который преобразует, или вызывает побочные эффекты, что приводит к непредвиденному поведению программы. \
